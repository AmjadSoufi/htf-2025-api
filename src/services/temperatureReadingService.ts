...
/**
 * Retrieves temperature readings across all sensors within an optional time range.
 * Returns flattened records with sensor id/lat/lon and reading temperature/timestamp.
 */
export const getTemperatureReadingsInRange = async (
  start?: Date,
  end?: Date
) => {
  const where: any = {};
  ...
  // Fetch readings and include sensor location for mapping/heatmaps
  ...
  return readings.map((r) => ({
    ...
    sensorId: r.temperatureSensorId,
    latitude: r.TemperatureSensor.latitude,
    longitude: r.TemperatureSensor.longitude,
  }));
};
